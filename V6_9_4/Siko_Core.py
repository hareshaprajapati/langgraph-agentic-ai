...
def print_locked_prediction_steps(
    ...
    *,
    ...
    rank_min_star = median([r["rank_min"] for r in rows])
    rank_max_star = median([r["rank_max"] for r in rows])
    rank_span_star = median([r["rank_span"] for r in rows])
    p_min_star = median([r["P_min"] for r in rows])
    p_max_star = median([r["P_max"] for r in rows])
    ...
    cohort_combos = []
    for combo in itertools.combinations(cohort_candidates, pred["K"] - 1):
        hwc = {"H": 0, "W": 0, "C": 0}
        ...
        for n in combo:
            ...
            max_p = max(max_p, m["P"])
        if (hwc["H"], hwc["W"], hwc["C"]) != (target_h, target_w, target_c):
            continue
        dec_tuple = tuple(dec[d] for d in decade_ids)
        if dec_tuple != target_dec_tuple:
            continue
