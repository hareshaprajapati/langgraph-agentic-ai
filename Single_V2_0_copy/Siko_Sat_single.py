...
RANDOM_SEED = 0
DEBUG_PRINT = True
# Strategy: "weighted", "hard_force", "cooccur", or "cluster"
STRATEGY_MODE = "cluster"
...
W_RANK = 0.10
COLD_BOOST = 0.45
# Co-occurrence weights (date-agnostic)
W_PAIR = 0.20
W_TRIPLE = 0.06
...
def _weighted_choice(items: List[Tuple[int, ...]], weights: List[float], rng: random.Random) -> Tuple[int, ...]:
...
def _cooccurrence_scores(draws: List[List[int]], order: int) -> Dict[int, int]:
    ...
    for nums in draws:
        if len(nums) < order:
            continue
        for comb in itertools.combinations(sorted(nums), order):
            combo_counts[comb] = combo_counts.get(comb, 0) + 1
    for comb, ct in combo_counts.items():
        for n in comb:
            counts[n] += ct
    return counts


def _top_combos(draws: List[List[int]], order: int, top_k: int) -> List[Tuple[Tuple[int, ...], int]]:
    if order < 2:
        return []
    ...
    ranked = sorted(counts.items(), key=lambda x: (-x[1], -sum(x[0])))
    return ranked[:top_k]


def _tight_cluster_pool(
    top_triples: List[Tuple[Tuple[int, ...], int]],
    top_pairs: List[Tuple[Tuple[int, ...], int]],
    cap: int,
) -> List[int]:
    pool = []
    for comb, _ in top_triples:
        for n in comb:
            if n not in pool:
                pool.append(n)
            if len(pool) >= cap:
                return pool
    for comb, _ in top_pairs:
        for n in comb:
            if n not in pool:
                pool.append(n)
            if len(pool) >= cap:
...
def score_numbers(df: pd.DataFrame, main_cols: List[str], target_date: str, debug: bool) -> List[CandidateScore]:
    ...
    recent_draws = train[(train["Date"] >= recent_start) & (train["Date"] < t)]
    recent_draw_list = [[int(row[c]) for c in main_cols] for _, row in recent_draws.iterrows()]
    ...
    s_pair = _normalize_series(pd.Series(_cooccurrence_scores(recent_draw_list, 2)).sort_index())
    s_triple = _normalize_series(pd.Series(_cooccurrence_scores(recent_draw_list, 3)).sort_index())
    ...
    for n in range(MAIN_MIN, MAIN_MAX + 1):
        ...
        total = (
            ...
            W_PAIR * float(s_pair.get(n, 0.0)) +
            W_TRIPLE * float(s_triple.get(n, 0.0))
        )
...
def generate_tickets(
    ...
    # co-occurrence combo pools (recent window)
    t = pd.Timestamp(target_date)
    recent_start = t - pd.Timedelta(days=LOOKBACK_DAYS)
    recent_draws = df[(df["Date"] >= recent_start) & (df["Date"] < t)]
    recent_draw_list = [[int(row[c]) for c in main_cols] for _, row in recent_draws.iterrows()]
    top_triples = _top_combos(recent_draw_list, 3, 40)
    top_pairs = _top_combos(recent_draw_list, 2, 60)
    cluster_pool = _tight_cluster_pool(top_triples, top_pairs, max(12, NUMBERS_PER_TICKET * 3))
    ...
    while len(tickets) < NUM_TICKETS and attempts < MAX_ATTEMPTS:
        ...
        if fixed_seed:
        ...
        elif strategy_mode == "cluster" and (top_triples or top_pairs):
            seed = []
            if top_triples:
                triples, triple_counts = zip(*top_triples)
                chosen = _weighted_choice(list(triples), list(triple_counts), rng)
                seed.extend([n for n in chosen if n not in seed])
            if top_pairs and len(seed) < NUMBERS_PER_TICKET:
                pairs, pair_counts = zip(*top_pairs)
                chosen = _weighted_choice(list(pairs), list(pair_counts), rng)
                seed.extend([n for n in chosen if n not in seed])
            seed = seed[:NUMBERS_PER_TICKET]
            remaining_k = NUMBERS_PER_TICKET - len(seed)
            remaining_items = [n for n in cluster_pool if n not in seed] or [n for n in pool if n not in seed]
            ...
            if remaining_k > 0 and remaining_items:
                ...
                rest = []
            pick = sorted(seed + rest)
        elif strategy_mode == "cooccur" and (top_triples or top_pairs):
            seed = []
            if top_triples:
                triples, triple_counts = zip(*top_triples)
                chosen = _weighted_choice(list(triples), list(triple_counts), rng)
...
if __name__ == "__main__":
    ...
    for _, row in last_three.iterrows():
        ...
        if STRATEGY_MODE == "cluster":
            mode_label = "CLUSTER"
        elif STRATEGY_MODE == "cooccur":
            mode_label = "COOCCUR"
        else:
