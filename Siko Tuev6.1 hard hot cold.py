...
# -*- coding: utf-8 -*-
"""
Hybrid Algo V6.1 for OZ Lotto Tuesday prediction.
...
TARGET_DRAWS_FOR_LEARNING = [
    ("OZ Lotto",         d(25,11)),
    ("Weekday Windfall", d(26,11)),  # NOTE: you must ensure data exists for this date
    ("Powerball",        d(27,11)),
...
def composition_targets(current_lottery_name, avg_hot, avg_warm, avg_cold,
    ...
    # --- 1) Fill up to draw_size by using spare capacity (original behaviour) ---
    while total() < draw_size:
        ...
        cat, cap_val = max(caps.items(), key=lambda kv: kv[1])

        if cap_val <= 0:
            # No category has spare capacity left
            break
    ...
    # --- 2) Enforce hard minimums so we never kill whole categories unintentionally ---

    # If we have any Cold numbers at all, don't allow c_target to be 0.
    # Steal a slot from the largest of Hot/Warm.
    if len(Cold_set) > 0 and c == 0 and total() == draw_size:
        if w >= h and w > 0:
            w -= 1
            c += 1
        elif h > 0:
            h -= 1
            c += 1
        # If both h and w are 0, then there are only cold numbers anyway, so leave as is.

    # If we have Hot numbers, try not to completely kill Hot category either.
    if len(Hot_set) > 0 and h == 0 and total() == draw_size:
        # Prefer stealing from Warm first, then Cold.
        if w > 1:
            w -= 1
            h += 1
        elif c > 1:
            c -= 1
            h += 1
        # If we can't steal without breaking everything, we leave it.

    # Final safety: clamp again to capacities
    h = min(h, len(Hot_set))
    w = min(w, len(Warm_set))
    c = min(c, len(Cold_set))

    if h < 0 or w < 0 or c < 0:
        raise RuntimeError("Composition targets went negative – check logic")

    if h + w + c != draw_size:
        # In extreme edge cases (tiny category sizes) we might not fill exactly.
        # Fall back to filling from any category with remaining capacity.
        # This keeps the implementation robust.
        while h + w + c < draw_size:
            cap_hot = len(Hot_set) - h
            cap_warm = len(Warm_set) - w
            cap_cold = len(Cold_set) - c
    ...
    # Final assert: must not exceed capacities
    if h > len(Hot_set) or w > len(Warm_set) or c > len(Cold_set):
...
def sample_from_category(category_list, probs_dict, k, chosen_set):
    """
    Sample k distinct numbers from category_list according to probs_dict,
    avoiding anything already in chosen_set.

    If we run out of candidates due to tight composition, we degrade
    gracefully instead of spinning or crashing.
    """
    selected = []
    while len(selected) < k:
        candidates = [n for n in category_list
                      if n not in chosen_set and n not in selected]
        if not candidates:
            # Nothing left to pick from this category – bail out gracefully.
            break
...
def monte_carlo_sampling(P, Hot_set, Warm_set, Cold_set,
    ...
    for _ in range(TRIALS):
        ...
        if len(chosen) != draw_size:
            # In extreme tight-capacity cases, we might come up short.
            # Skip this trial instead of crashing.
            continue
