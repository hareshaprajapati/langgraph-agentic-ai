...
log_file_path = os.path.join(".", "Siko_Sat_5_hit.py.log")
...
def main() -> None:
    ...
    for d in bt_dates:
        ...
        for idx, t in enumerate(best_tickets[:3], 1):
            d = _ticket_diagnostics(t, num_scores, pair_pmi if 'pair_pmi' in locals() else {}, triplet_counts if 'triplet_counts' in locals() else Counter(), dist, [], [])
