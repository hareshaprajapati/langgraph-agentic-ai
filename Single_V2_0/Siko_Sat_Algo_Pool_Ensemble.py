...
def recent_score(n, f):
    # Simplified recent score using available features
    return (
        0.5 * f['freq_no40_10'].get(n, 0)
        - 0.15 * f['gap_no40'].get(n, 0)
    )

def recent_linear5_score(n, f, no40_list):
    score = 0
    last5 = no40_list[-5:]
    for i, nums in enumerate(reversed(last5)):
        if n in nums:
            score += (5 - i)
    return score
...
def build_fixed_ensemble_pool(f, t, last_draw_nums, prior_no40):
    """Fixed Ensemble of 5 strategies."""
    no40_list = list(prior_no40['nums'])
    caps = {'0s': 6, '10s': 3, '20s': 3, '30s': 3}

    # 1. Original buggy
    pool1 = build_pool_original_buggy(f, t, last_draw_nums)

    # 2. Corrected legacy, caps 6/3/3/3, max_prev=2
    pool2 = build_pool_from_score(
        lambda n: legacy_score(n, f, t, BEST_WEIGHTS),
        f, t, last_draw_nums, caps, max_prev=2
    )

    # 3. Cap-search legacy, caps 5/4/3/2, max_prev=2
    ...
    # 4. Recent score
    pool4 = build_pool_from_score(
        lambda n: recent_score(n, f),
        f, t, last_draw_nums, caps, max_prev=2
    )

    # 5. Recent linear5
    def lin_score(n):
        return recent_linear5_score(n, f, no40_list)
    pool5 = build_pool_from_score(
        lin_score,
        f, t, last_draw_nums, caps, max_prev=2
    )

    # Frequency vote across all 5 pools
    freq = Counter()
    for p in [pool1, pool2, pool3, pool4, pool5]:
        freq.update(p)

    def final_score(n):
        return 1000.0 * freq.get(n, 0) + legacy_score(n, f, t, BEST_WEIGHTS)
...
def run_backtest(n_test=20):
    ...
    print(f"Fixed Ensemble backtest on last {n_test} no-40 draws...\n")
    print("=" * 90)
    ...
    for idx, entry in enumerate(target_cache, 1):
        ...
        prior_no40 = no40_df[no40_df["Date_dt"] < date]
        pool = build_fixed_ensemble_pool(f, t, last_draw, prior_no40)
    ...
    print("\n" + "=" * 90)
    print(f"FIXED ENSEMBLE BACKTEST RESULT (LAST {n_test} NO-40 DRAWS)")
    print("=" * 90)
...
def predict_for_date(date_str):
    ...
    print(f"Generating Fixed Ensemble pool for {target_date.strftime('%d-%b-%Y')}...\n")
    ...
    pool = build_fixed_ensemble_pool(features, tiers, last_draw, prior_no40)
