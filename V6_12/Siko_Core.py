...
BASE_TRIALS = 300         # base TRIALS factor before scaling
MIN_TRIALS = 600          # minimum MC trials
MAX_TRIALS = 900         # maximum MC trials
...
def build_rank_rows_from_res_pred(res_pred, exclude_hot=False):
    """
    Build rank_rows from res_pred maps only (P/log_score + Hot/Warm/Cold sets).
    Returns list of dict rows:
      {"rank":int, "n":int, "dec":int, "cat":"H|W|C", "log_score":float, "P":float}
    """
    P_map = res_pred.get("P") or res_pred.get("P_map") or {}
    log_map = res_pred.get("log_score") or res_pred.get("log_score_map") or {}

    hot_set = set(res_pred.get("Hot_set") or [])
    warm_set = set(res_pred.get("Warm_set") or [])
    cold_set = set(res_pred.get("Cold_set") or [])

    # If maps are missing/empty, fail loudly so you don't "silently" get no tickets
    if not isinstance(P_map, dict) or not P_map:
        raise ValueError("build_rank_rows_from_res_pred(): missing/empty res_pred['P'] (or P_map)")
    if not isinstance(log_map, dict) or not log_map:
        raise ValueError("build_rank_rows_from_res_pred(): missing/empty res_pred['log_score'] (or log_score_map)")
    ...
    for n, P in P_map.items():
        ...
        if exclude_hot and (n_int in hot_set):
            continue

        if n_int in hot_set:
            cat = "H"
        elif n_int in warm_set:
            cat = "W"
        elif n_int in cold_set:
            cat = "C"
        else:
            # If it isn't in any bucket, treat as Warm (neutral) to avoid dropping it
            cat = "W"

        ls = float(log_map.get(n_int, log_map.get(str(n_int), 0.0)))
        rows.append({
            "rank": None,
            "n": n_int,
            "dec": decade_of(n_int),
...
def pick_best_p_window(rank_rows, window_len=9, bonus=0.001, preferred_start_rank=None, search_radius=6):
    """
    Picks the best window of length window_len from rank_rows.

    - If preferred_start_rank is provided, it FIRST tries to pick from starts near it
      (within +/- search_radius) before falling back to global best.
    - Score = sum(P) + (decade_coverage * bonus)
    """
    n = len(rank_rows)
    if n < window_len:
        return None
    ...
    def score_window(rows):
        ...
        decades = len(set(r["dec"] for r in rows))
        return sumP + decades * bonus, sumP, decades

    best = None

    # 1) preferred search band (if provided)
    if preferred_start_rank is not None:
        start_lo = max(1, preferred_start_rank - search_radius)
        start_hi = min(n - window_len + 1, preferred_start_rank + search_radius)

        for start in range(start_lo, start_hi + 1):
            rows = rank_rows[start - 1:start - 1 + window_len]
            sc, sumP, decades = score_window(rows)
            cand = {
                "start_rank": start,
                "end_rank": start + window_len - 1,
    ...
    for start in range(1, n - window_len + 2):
        rows = rank_rows[start - 1:start - 1 + window_len]
        sc, sumP, decades = score_window(rows)
        cand = {
            "start_rank": start,
...
def generate_tickets_from_window(
    ...
    *,
    ...
    """
    Generate tickets (6-number combos) from a candidate pool window_nums.

    Returns list[dict] where each dict has:
      {"nums": [...], "prodP": float, "sumLog": float, "hwc": (H,W,C), "dec": {1:..}}
    """
    ...
    # If cat_of not provided, cannot compute hwc constraints
    if (target_hwc is not None) and (cat_of is None):
        # fail safe: ignore hwc constraint if we can't compute it
        target_hwc = None

    combos_iter = itertools.combinations(pool, 6)

    # To avoid huge explosion if pool gets large
    combos = []
    for i, c in enumerate(combos_iter):
        combos.append(c)
        if i >= max_combos:
            break
    ...
    for c in combos:
        ...
        if target_dec is not None:
            dc = Counter(decade_of(n) for n in c)
            # normalize missing keys
            dc_full = {k: dc.get(k, 0) for k in range(1, 6)}
            if any(dc_full[k] != target_dec.get(k, 0) for k in range(1, 6)):
                continue

        if target_hwc is not None:
            h = sum(1 for n in c if cat_of(n) == "H")
            w = sum(1 for n in c if cat_of(n) == "W")
            cold = sum(1 for n in c if cat_of(n) == "C")
            if (h, w, cold) != tuple(target_hwc):
                continue
        else:
        ...
        sumLog = 0.0
        for n in c:
            prodP *= float(P_map.get(n, 0.0))
            sumLog += float(log_score_map.get(n, 0.0))
    ...
    # sort: strongest prodP, then sumLog
    scored.sort(key=lambda t: (t["prodP"], t["sumLog"]), reverse=True)
...
def build_tickets_from_run_payload(
    ...
    *,
    ...
    target_hwc=None,
    target_dec=None,
    learned_start_rank=None,
    search_buckets=5,   # IMPORTANT: expands around learned_start_rank
    bonus=0.001,
):
    """
    Core ticket build:
    - Build rank_rows
    - Choose window near learned_start_rank (if provided)
    - If constraints impossible, expand candidate pool by adding neighboring windows (buckets)
    """
    res_pred = run_payload["res_pred"]

    # Required maps
    P_map = res_pred.get("P") or res_pred.get("P_map")
    log_score_map = res_pred.get("log_score") or res_pred.get("log_score_map")
    if not P_map or not log_score_map:
        raise ValueError("Missing P/log_score maps in res_pred (expected keys: 'P'/'log_score' or 'P_map'/'log_score_map').")

    # Build rank rows from existing prints
    rank_rows = build_rank_rows_from_res_pred(res_pred, exclude_hot=exclude_hot)

    # Pick best window (near learned start rank if supplied)
    best_window = pick_best_p_window(
        rank_rows,
        window_len=window_len,
        bonus=bonus,
        preferred_start_rank=learned_start_rank,
        search_radius=max(6, search_buckets * 2),
    )

    if best_window is None:
        return {"best_window": None, "tickets": [], "reason": "No window possible (rank_rows too short)."}

    # candidate pool expansion:
    # start with best_window, then union more windows around its start to satisfy constraints
    candidate_nums = set(best_window["nums"])

    # helper: decade + category functions
    decade_of = lambda n: next((d for (d, lo, hi) in DECADE_BANDS if lo <= n <= hi), None)

    Hot_set = set(res_pred.get("Hot_numbers", res_pred.get("Hot_set", [])))
    Warm_set = set(res_pred.get("Warm_numbers", res_pred.get("Warm_set", [])))
    Cold_set = set(res_pred.get("Cold_numbers", res_pred.get("Cold_set", [])))

    def cat_of(n):
        if n in Hot_set: return "H"
        if n in Warm_set: return "W"
        if n in Cold_set: return "C"
        return "?"

    # Expand by additional buckets around learned_start_rank if needed
    def add_window_at_start(start):
        if start < 1:
            return
        end = start + window_len - 1
        if end > len(rank_rows):
            return
        for r in rank_rows[start - 1:end]:
            candidate_nums.add(r["n"])

    if learned_start_rank is not None:
        # Add symmetric buckets around learned_start_rank
        for k in range(1, search_buckets + 1):
            add_window_at_start(learned_start_rank - k)
            add_window_at_start(learned_start_rank + k)

    # Try strict first
    tickets = generate_tickets_from_window(
        sorted(candidate_nums),
        P_map=P_map,
    ...
    # If still empty, relax in controlled order (logged)
    relax_log = []
    if not tickets:
        relax_log.append("STRICT constraints produced 0 tickets; relaxing target_hwc first.")
        tickets = generate_tickets_from_window(
            sorted(candidate_nums),
            P_map=P_map,
            log_score_map=log_score_map,
            ...
            n_tickets=n_tickets,
            seed=seed,
            target_hwc=None,            # relax
            target_dec=target_dec,
            cat_of=cat_of,
        )

    if not tickets:
        relax_log.append("Relaxed HWC still produced 0; relaxing target_dec too.")
        tickets = generate_tickets_from_window(
            sorted(candidate_nums),
            P_map=P_map,
            log_score_map=log_score_map,
    ...
    return {
        ...
        "candidate_pool_size": len(candidate_nums),
        "relax_log": relax_log,
    }
...
def _window_has_required_decades(win_rows, target_dec):
    """Feasibility check: window must contain at least required count per decade."""
    ...
    for d, need in target_dec.items():
        if have.get(d, 0) < need:
            return False
    return True


def _window_has_required_hwc(win_rows, target_hwc):
    """Feasibility check: window must contain at least required count per H/W/C."""
    if target_hwc is None:
        return True
    h_need, w_need, c_need = target_hwc
    h = sum(1 for r in win_rows if r["cat"] == "H")
    w = sum(1 for r in win_rows if r["cat"] == "W")
    c = sum(1 for r in win_rows if r["cat"] == "C")
    return (h >= h_need) and (w >= w_need) and (c >= c_need)


def pick_best_p_window_guided(
    rank_rows,
    *,
    window_len=9,
    bonus=0.001,
    preferred_start_rank=None,     # e.g. learned_start_rank
    rank_penalty=0.002,            # weight for distance penalty
    target_dec=None,
    target_hwc=None,
):
    """
    Pick a window that is:
      - feasible for target_dec/target_hwc (at least "contains enough" to build tickets)
      - high sum(P) + decade_coverage*bonus
      - close to preferred_start_rank (if provided)
    Returns same shape as pick_best_p_window().
    """
    if not rank_rows or len(rank_rows) < window_len:
        return None

    best = None
    for i in range(0, len(rank_rows) - window_len + 1):
        win = rank_rows[i:i + window_len]

        # Feasibility gates (critical fix)
        if not _window_has_required_decades(win, target_dec):
            continue
        if not _window_has_required_hwc(win, target_hwc):
            continue

        nums = [r["n"] for r in win]
        sumP = sum(r["P"] for r in win)
        decades = len(set(r["dec"] for r in win if r["dec"] is not None))
        score = sumP + decades * bonus

        if preferred_start_rank is not None:
            dist = abs(win[0]["rank"] - int(preferred_start_rank))
            score -= rank_penalty * dist
...
def _count_hwc_in_rows(rows):
    h = sum(1 for r in rows if r.get("cat") == "H")
    w = sum(1 for r in rows if r.get("cat") == "W")
    c = sum(1 for r in rows if r.get("cat") == "C")
    return h, w, c


def _window_has_required_hwc(rows, target_hwc):
    if not target_hwc:
        return True
    h_need, w_need, c_need = target_hwc
    h_av, w_av, c_av = _count_hwc_in_rows(rows)
    return (h_need <= h_av) and (w_need <= w_av) and (c_need <= c_av)


def _window_has_required_decades(rows, target_dec):
    if not target_dec:
        return True
    dc = {}
    for r in rows:
...
def _closest_feasible_hwc(target_hwc, rows, draw_size):
    """
    Project target H/W/C onto what's feasible inside this window (same draw_size).
    Deterministic: minimal L1 distance, tiebreak prefer more W then more H.
    """
    if not target_hwc:
        return None
    ...
    for h in range(0, min(h_av, draw_size) + 1):
        for w in range(0, min(w_av, draw_size - h) + 1):
            c = draw_size - h - w
            if c < 0 or c > c_av:
                continue
            dist = abs(h - ht) + abs(w - wt) + abs(c - ct)
            key = (dist, -w, -h)
...
def pick_best_p_window_guided(
    ...
    *,
    window_len=9,
    bonus=0.001,
    preferred_start_rank=None,
    start_rank_penalty=0.001,
    target_hwc=None,
    target_dec=None,
):
    """
    Base score: sum(P) + bonus*decade_coverage
    Guidance: subtract start_rank_penalty*abs(start_rank - preferred_start_rank)

    Selection order:
      1) windows feasible for BOTH target_hwc and target_dec
      2) relax HWC only (keep decade feasibility)
      3) relax everything (still guided by preferred_start_rank)
    """
    N = len(rank_rows)
    if N < window_len:
        return None, "rank_rows shorter than window_len"

    def score_window(i):
        rows = rank_rows[i:i + window_len]
        sumP = sum(r["P"] for r in rows)
        decades = len({r["decade"] for r in rows})
        score = sumP + bonus * decades
        if preferred_start_rank is not None:
            score -= start_rank_penalty * abs(rows[0]["rank"] - preferred_start_rank)
        return score, sumP, decades, rows
    ...
    for i in range(0, N - window_len + 1):
        score, sumP, decades, rows = score_window(i)
        if not _window_has_required_hwc(rows, target_hwc):
            continue
        if not _window_has_required_decades(rows, target_dec):
            continue
    ...
    # pass 2: relax hwc only
    best = None
    best_score = None
    for i in range(0, N - window_len + 1):
        score, sumP, decades, rows = score_window(i)
        if target_dec is not None and (not _window_has_required_decades(rows, target_dec)):
            continue
        if best is None or score > best_score:
            best = {
                "start_rank": rows[0]["rank"],
