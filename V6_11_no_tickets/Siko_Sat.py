...
if __name__ == "__main__":
    ...
    # Pull maps from the FINAL run payload (the run you are building tickets for)
    res_pred = final_run.get("res_pred", {}) if isinstance(final_run, dict) else {}
    P_map = res_pred.get("P_map") or res_pred.get("P") or {}
    log_map = res_pred.get("log_score_map") or res_pred.get("log_score") or {}


    def _ticket_metrics(nums):
        prodP = 1.0
        sumLog = 0.0
        for n in nums:
            n = int(n)
            prodP *= float(P_map.get(n, 0.0))
            sumLog += float(log_map.get(n, 0.0))
        return prodP, sumLog


    for i, t in enumerate(result.get("tickets", []), start=1):
        # Support BOTH formats:
        #   - ticket is dict: {"nums":[...], "prodP":..., "sumLog":...}
        #   - ticket is list/tuple: [...]
        if isinstance(t, dict):
            nums = t.get("nums", [])
            prodP = t.get("prodP")
            sumLog = t.get("sumLog")
            if prodP is None or sumLog is None:
                prodP, sumLog = _ticket_metrics(nums)
        else:
            nums = list(t)
            prodP, sumLog = _ticket_metrics(nums)

        print(f"T#{i:02d} {nums}  prodP={prodP:.12f}  sumLog={sumLog:.6f}")
