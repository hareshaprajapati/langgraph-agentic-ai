...
# -*- coding: utf-8 -*-
"""
Hybrid Algo V5.1 for OZ Lotto Tuesday prediction.
...
BASE_TRIALS = 60000         # base TRIALS factor before scaling
MIN_TRIALS = 40000          # minimum MC trials
MAX_TRIALS = 120000         # maximum MC trials
...
def compute_recent_decade_weights(target_date,
    ...
    """
    New decade engine:
    - Look at last `recent_days` calendar days before target_date.
    - Count main + supp + powerball (if in 1..47) across ALL lotteries.
    - Auto-compute suppression/boost per decade:
        - Over-used decades -> weight down
        - Under-used decades -> weight up
    - No hard thresholds.
    Returns:
        decade_weight_log[n],
        dec_short_counts[d_id],
        dec_factors[d_id]  (normalised decade multipliers)
    """
    start_date = target_date - datetime.timedelta(days=recent_days)
    end_date = target_date - datetime.timedelta(days=1)

    # 1) count by decade
    dec_short_counts = {d_id: 0 for d_id in DECADES}
    ...
    for dt, draws in draws_by_date.items():
        ...
        for dr in draws:
            ...
            for n in nums:
                d_id = decade_of(n)
                if d_id is not None:
                    dec_short_counts[d_id] += 1

    total = sum(dec_short_counts.values())
    # If nothing to learn from, just neutral decades
    if total == 0:
    ...
    # 2) normalise counts to [0,1] for each decade
    C_min = min(dec_short_counts.values())
    C_max = max(dec_short_counts.values())
    eps = 1e-9

    raw = {}
    for d_id, C_d in dec_short_counts.items():
        t = (C_d - C_min) / (C_max - C_min + eps)   # 0 = least used, 1 = most used
        # Exponential interpolation between max boost and max suppression
        raw[d_id] = RECENT_DECADE_W_MAX * (RECENT_DECADE_W_MIN / RECENT_DECADE_W_MAX) ** t

    # 3) renormalise so average factor is 1.0
    avg_raw = sum(raw.values()) / len(raw)
    if avg_raw <= 0:
        dec_factors = {d_id: 1.0 for d_id in DECADES}
    else:
        dec_factors = {d_id: raw[d_id] / avg_raw for d_id in DECADES}

    # 4) map to per-number log weights
    decade_weight_log = {}
...
def per_number_log_scores(seed_numbers_main, f_main, f_supp, L_counts,
    ...
    for n in NUMBER_RANGE:
        ls = (
                ...
                delta_log[n] +  # last-3-days all lotteries
                sfl_repeat_log[n] +  # NEW: SFL exact-repeat momentum
                sfl_adj_log[n] +  # NEW: SFL adjacency momentum (±1, ±2)
                cross_log[n] +  # cross-lottery density (L_counts)
                supp_log[n] +  # supp-only bonus
                category_weight_log[n] +  # H/W/C weighting
                decade_weight_log[n] +  # (unchanged, even if we "ignore" decades conceptually)
                cross_hop_log[n] +  # cross-lottery hop (now stronger SFL→OZ)
                cold_resurge_log[n]  # cold resurgence
        )
