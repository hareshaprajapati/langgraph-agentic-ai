...
# -*- coding: utf-8 -*-
"""
Hybrid Algo V4 for OZ Lotto Tuesday prediction.
- Combines original wide-exploration behaviour (strong near-miss spread)
- With new structural features: decades, cold resurgence, centre vs neighbour,
  cross-lottery hop, and learning on H/W/C + decades + clusters.

You can adjust only the data section and target configs; rest is the core engine.
"""
...
BASE_TRIALS = 60000      # base factor for TRIALS scaling
...
# --- DATA (as provided earlier; adjust as needed) ---
...
TARGET_DRAWS_FOR_LEARNING = [
    ...
    ("Weekday Windfall", d(1,12)),  # adjust if you have data
]
...
def step_B_decade(seed_draws_dec):
    ...
    if total_main_in_range_dec == 0:
        ...
        dec_freq = {}
        for d_id in DECADES:
            dec_freq[d_id] = dec_main_count[d_id] / total_main_in_range_dec
...
def decade_momentum(dec_w_final, dec_main_count, total_main_in_range_dec,
    ...
    if total_recent > 0:
        epsilon = 1e-6
        for d_id in DECADES:
            dec_recent_freq = dec_recent_count[d_id] / total_recent
...
# ===========================
# Step 10: Monte Carlo sampling (V4)
# ===========================
...
def monte_carlo_sampling(P, Hot_set, Warm_set, Cold_set,
                         ...
                         centre_score):
    # TRIALS computation (V4: slightly higher range)
    complexity = math.sqrt(max(1.0, M / 20.0))
    TRIALS = int(clamp(BASE_TRIALS * complexity, 30000, 60000))

    max_avg = max(avg_hot, avg_warm, avg_cold)
    # V4: slightly higher exploration range
    EXPLORE_FRAC = clamp(0.14 + 0.18 * max(0.0, max_avg - 1/3), 0.14, 0.26)
    ...
    # V4: centre-biased ranking
    tuple_score = {}
    for T, p in prob.items():
        cb = tuple_centre_bias.get(T, 0.5)
        # normalise around 0.5
        factor = 1.0 + ALPHA_CENTRE * (cb - 0.5)
        if factor < 0.5:
            factor = 0.5
        tuple_score[T] = p * factor

    top10_tuples = sorted(prob.keys(), key=lambda T: tuple_score[T], reverse=True)[:10]
    top10 = []
    for T in top10_tuples:
        top10.append((T, tuple_score[T], prob[T]))
...
def main():
    ...
    print("\n=== FINAL PREDICTION (V4 Hybrid) ===")
    print(f"Lottery: {prediction_lottery_name}")
    print(f"Date:    {prediction_date}")
    print(f"Draw size: {prediction_draw_size}")
    print("\nTop-10 predicted tuples (centre-biased score, base prob):")
    for T, score, p in res_pred["top10"]:
        print(f"{list(T)}  score={score:.6f}  prob={p:.6f}")
