...
def hwc_counts(nums, Hot_set, Warm_set, Cold_set):
    """Return (H, W, C) counts for a list/tuple of numbers."""
...
def decade_counts(nums):
    """Return {decade_id: count} for the given numbers."""
    decs = {d_id: 0 for d_id in DECADES}
    for n in nums:
        d_id = decade_of(n)
        if d_id is not None:
            decs[d_id] += 1
    # strip zero decades for nicer printing (optional)
    return {d_id: cnt for d_id, cnt in decs.items() if cnt > 0}
...
def main():
    ...
    # Main run with chosen windows + detailed diagnostics (V4.1 style)
    state = LearningState()
    for lottery_name, target_date in TARGET_DRAWS_FOR_LEARNING:
        res = process_target(lottery_name, target_date,
        ...
        print(f"[MAIN RUN] {lottery_name} on {target_date}")
        if actual_main is None:
            print("  No actual draw available for verification\n")
            continue
        ...
        # Actual H/W/C + decades
        h_act, w_act, c_act = hwc_counts(actual_main, Hot_set, Warm_set, Cold_set)
        dec_act = decade_counts(actual_main)

        # Model targets and MC averages
        h_tgt = res["h_target"]
        w_tgt = res["w_target"]
        c_tgt = res["c_target"]

        hot_pred = res["hot_pred"]
        warm_pred = res["warm_pred"]
        cold_pred = res["cold_pred"]
        dec_pred = res["dec_pred"]

        print(f"  Actual main: {actual_main}")
        print(f"  Actual H/W/C: H={h_act}, W={w_act}, C={c_act}")
        print(f"  Actual decades: {dec_act}")
        print(f"  Model H/W/C target: h_target={h_tgt}, w_target={w_tgt}, c_target={c_tgt}")
        print(f"  MC H/W/C avg: hot={hot_pred:.3f}, warm={warm_pred:.3f}, cold={cold_pred:.3f}")
        print(f"  MC decades predicted (avg counts): {{"
              + ", ".join(f"{d}: {dec_pred[d]:.3f}" for d in sorted(dec_pred.keys()))
              + "}}")
        print()
    ...
    print("=== FINAL PREDICTION (V4.1 Hybrid-style) ===")
    print(f"Lottery:   {prediction_lottery_name}")
    print(f"Date:      {prediction_date}")
    print(f"Draw size: {prediction_draw_size}")

    TRIALS = res_pred["TRIALS"]
    EXPLORE_FRAC = res_pred["EXPLORE_FRAC"]
    print(f"TRIALS:    {TRIALS}, EXPLORE_FRAC={EXPLORE_FRAC:.3f}\n")

    # Hot / Warm / Cold lists
    print("Hot numbers: ", sorted(Hot_set))
    print("Warm numbers:", sorted(Warm_set))
    print("Cold numbers:", sorted(Cold_set))

    hot_pred = res_pred["hot_pred"]
    warm_pred = res_pred["warm_pred"]
    cold_pred = res_pred["cold_pred"]
    dec_pred = res_pred["dec_pred"]

    print(f"MC H/W/C avg: hot={hot_pred:.3f}, warm={warm_pred:.3f}, cold={cold_pred:.3f}")
    print("MC decades predicted (avg counts): {"
          + ", ".join(f"{d}: {dec_pred[d]:.3f}" for d in sorted(dec_pred.keys()))
          + "}\n")

    # Top-10 tuples with H/W/C + decades, and "score" + "prob"
    print("Top-10 predicted tuples (joint score, base prob, H/W/C, decades):")
    for T, p in res_pred["top10"]:
        h_t, w_t, c_t = hwc_counts(T, Hot_set, Warm_set, Cold_set)
        dec_t = decade_counts(T)
        score = p  # using p directly
        print(f"{list(T)}  score={score:.6f}  prob={p:.6f}  H/W/C=({h_t},{w_t},{c_t})  decades={dec_t}")
