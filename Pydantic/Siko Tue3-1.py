...
TARGET_DRAWS_FOR_LEARNING = [
    ...
    ("Weekday Windfall", d(1,12)),   # you'll fill data later
]

PREDICTION_TARGET = ("OZ Lotto", d(2,12), 7)  # you'll adjust if needed
...
# (now including centre vs neighbour)
# ===========================
...
def per_number_log_scores(seed_numbers_main, f_main, f_supp, L_counts,
    ...
    # --- Centre vs neighbour recency (near-miss fix) ---
    ...
    for dt, draws in draws_by_date.items():
        if last3_start <= dt <= target_date - datetime.timedelta(days=1):
            for dr in draws:
                for x in dr.main:
                    if MAIN_NUMBER_MIN <= x <= MAIN_NUMBER_MAX:
                        if x - 1 >= MAIN_NUMBER_MIN:
                            neigh_hits_last3[x - 1] += 1
                        if x + 1 <= MAIN_NUMBER_MAX:
                            neigh_hits_last3[x + 1] += 1

    centre_raw = {}
    for n in NUMBER_RANGE:
        centre_raw[n] = (1 if main_hits_last3[n] else 0) - 0.7 * neigh_hits_last3[n]
    ...
    # cold resurgence (exclude hot set)
    ...
    return P, log_score, cold_resurge_score, centre_score
...
def process_target(lottery_name, target_date,
    ...
    # per-number scores including centre vs neighbour
    P, log_score, cold_resurge_score, centre_score = per_number_log_scores(
    ...
    result = {
        ...
        "f_main": f_main,
        "clusters": clusters,
        "centre_score": centre_score,   # <- for centre-biased ranking
        "prob": mc_diag["prob"],        # <- all tuple probabilities
    }
...
# (with centre-biased final ranking)
# ===========================
...
def main():
    ...
    # Main run with chosen windows (learning)
    state = LearningState()
    for lottery_name, target_date in TARGET_DRAWS_FOR_LEARNING:
        res = process_target(lottery_name, target_date,
    ...
    prob = res_pred["prob"]
    centre_score = res_pred["centre_score"]
    alpha_centre = 0.25  # centre-bias strength
    ...
    top10 = sorted(tuple_scores.items(), key=lambda kv: kv[1], reverse=True)[:10]

    print("\nTop-10 predicted tuples (centre-biased score, base prob):")
    for T, score in top10:
        print(f"{list(T)}  score={score:.6f}  prob={prob[T]:.6f}")
