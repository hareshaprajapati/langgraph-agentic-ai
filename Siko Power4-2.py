...
# -*- coding: utf-8 -*-
"""
Hybrid Algo V4.1 for OZ Lotto Tuesday prediction.
- Builds on V4 with:
  * Stronger cluster usage
  * Conditional sampling via cluster-first trials
  * Tie-break on joint likelihood (cluster cohesion + centre bias)
  * Bigger MC trials when structure is confident
- Prints H/W/C + decade breakdown for:
  * All TARGET_DRAWS_FOR_LEARNING (actual vs model)
  * All top-N predicted tuples for the final prediction
...
PREDICTION_TARGET = ("Powerball", d(3,12), 7)
...
def decade_momentum(dec_w_final, dec_main_count, total_main_in_range_dec,
    ...
    for n in NUMBER_RANGE:
        d_id = decade_of(n)
        if d_id is not None:
            decade_weight_log[n] = 0.7 * math.log(dec_w_recent[d_id])
        else:
