...
# PREDICTION_TARGET: tuple[str, Any, int]
...
# APPLY_PREDICTION_OVERRIDES = True     # Run-2: True, Run-1: False
...
def _make_run_payload(
    *,
    ...
    return {
        ...
        "date": target_date,  # keep old name too
        "draw_size": draw_size,
        "res_pred": res_pred,
        ...
        # normalized keys
        "P_map": P_map if P_map is not None else (res_pred.get("P") or {}),
        "log_score_map": log_score_map if log_score_map is not None else (res_pred.get("log_score") or {}),
        "rank_rows": rank_rows if rank_rows is not None else (res_pred.get("rank_rows") or []),
        "ranked": ranked if ranked is not None else (res_pred.get("ranked") or []),
    }

def _decade_of_number(n: int):
    for dec, lo, hi in DECADE_BANDS:
        if lo <= n <= hi:
            return dec
    return None

def _hwc_of_number(n: int, hot_set, warm_set, cold_set):
    if n in hot_set:
        return "H"
    if n in warm_set:
        return "W"
...
def _build_rank_rows_from_maps(P_map, log_score_map, hot_set, warm_set, cold_set):
    """
    Returns:
      rank_rows: list of tuples (rank1, n, decade, hwc, log_score, P) sorted by P desc
      rank_of:   dict n -> rank1
    """
    ranked_nums = sorted(P_map.keys(), key=lambda k: P_map.get(k, 0.0), reverse=True)
    rank_rows = []
    for r, n in enumerate(ranked_nums, start=1):
        rank_rows.append((
            r,
            n,
            _decade_of_number(n),
            _hwc_of_number(n, hot_set, warm_set, cold_set),
            float(log_score_map.get(n, 0.0)),
            float(P_map.get(n, 0.0)),
        ))
    rank_of = {n: r for (r, n, *_rest) in rank_rows}
    return rank_rows, rank_of
...
def learn_pwindow_mappings_from_actuals(
    ...
    *,
    ...
    """
    Learn which consecutive P-rank windows tend to contain the ACTUAL numbers.

    Each run in `runs` must contain:
      - 'rank_rows' (or 'P_map' etc)
      - 'rank_of'
      - 'actual_main' (list[int])  (skip run if missing)
      - 'Hot_set','Warm_set','Cold_set' (for H/W/C classification within that run)

    Returns dict with per-window-length stats:
      learned['by_len'][L]['starts'] : Counter(start_rank -> count)
      learned['by_len'][L]['hwc_modes'] : Counter((H,W,C) -> count) for best window per run
      learned['by_len'][L]['dec_modes'] : Counter((d1,d2,d3,d4,d5) -> count) for best window per run
    """
    import collections

    def near(a, b):  # ±1 by default
        return abs(a - b) <= near_miss_delta
    ...
    for L in window_lens:
        learned["by_len"][L] = {
            ...
            "best_windows": [],
            "hwc_modes": collections.Counter(),
            "dec_modes": collections.Counter(),
        }
    ...
    for run in runs:
        ...
        if not rank_rows:
            print("run", run)
            P_map = run["P_map"]
            log_score_map = run["log_score_map"]
            hot_set = set(run.get("Hot_set") or [])
            warm_set = set(run.get("Warm_set") or [])
            cold_set = set(run.get("Cold_set") or [])
            rank_rows, rank_of = _build_rank_rows_from_maps(P_map, log_score_map, hot_set, warm_set, cold_set)
            run["rank_rows"] = rank_rows
            run["rank_of"] = rank_of

        ranked_nums = [row[1] for row in rank_rows]
        ...
        for L in window_lens:
            if len(ranked_nums) < L:
                continue

            best = None  # (exact, near_hits, start_rank, window_nums, dc_tuple, hwc_tuple)
            for start_idx in range(0, len(ranked_nums) - L + 1):
                window = ranked_nums[start_idx:start_idx + L]
                wset = set(window)

                exact = len(actual_set & wset)
                near_hits = sum(1 for a in actual if any(near(a, w) for w in window))

                # decade composition
                dc = {d: 0 for d, _, _ in DECADE_BANDS}
                for n in window:
                    dc[_decade_of_number(n)] += 1
                dc_tuple = tuple(dc[d] for d, _, _ in DECADE_BANDS)

                # H/W/C composition (using run's sets)
                H = sum(1 for n in window if n in hot_set)
                W = sum(1 for n in window if n in warm_set)
                C = L - H - W
                hwc_tuple = (H, W, C)

                cand = (exact, near_hits, start_idx + 1, window, dc_tuple, hwc_tuple)
                if best is None or (cand[0], cand[1]) > (best[0], best[1]):
                    best = cand

            if best:
                exact, near_hits, start_rank, window, dc_tuple, hwc_tuple = best
                learned["by_len"][L]["starts"][start_rank] += 1
                learned["by_len"][L]["hwc_modes"][hwc_tuple] += 1
                learned["by_len"][L]["dec_modes"][dc_tuple] += 1
                learned["by_len"][L]["best_windows"].append({
                    "date": run.get("date"),
                    "lottery": run.get("lottery"),
                    "start_rank": start_rank,
                    "end_rank": start_rank + L - 1,
                    "exact": exact,
                    "near": near_hits,
                    "nums": list(window),
                    "hwc": hwc_tuple,
...
def pick_prediction_window_from_history(
    final_run,
    learned,
    *,
    window_len=9,
    ...
    """
    Uses learned start-rank votes to pick a likely window on the final run.
    Returns dict: {start_rank,end_rank,nums,sumP,hwc,dec}
    """


    rank_rows = final_run.get("rank_rows") or []
    if not rank_rows:
        raise ValueError("final_run has no rank_rows. Ensure core.main() includes rank_rows in payload.")

    ranked_nums = [row[1] for row in rank_rows]
    P_map = final_run["P_map"]

    start_votes = learned["by_len"][window_len]["starts"]
    start_candidates = [s for s, _ in start_votes.most_common(candidates)]
    if not start_candidates:
        # fallback: scan early ranks
        start_candidates = list(range(1, min(40, len(ranked_nums) - window_len + 2)))

    hot_set = set(final_run.get("Hot_set") or [])
    warm_set = set(final_run.get("Warm_set") or [])
    cold_set = set(final_run.get("Cold_set") or [])

    best = None
    for s in start_candidates:
        i = s - 1
        if i < 0 or i + window_len > len(ranked_nums):
            continue

        window = ranked_nums[i:i + window_len]
        sumP = sum(P_map.get(n, 0.0) for n in window)

        # decade + hwc
        dc = {d: 0 for d, _, _ in DECADE_BANDS}
        for n in window:
            dc[_decade_of_number(n)] += 1
        dec_tuple = tuple(dc[d] for d, _, _ in DECADE_BANDS)
        ...
        cand = (sumP, s, window, hwc_tuple, dec_tuple)
    ...
    sumP, s, window, hwc_tuple, dec_tuple = best
    return {
        "start_rank": s,
        "end_rank": s + window_len - 1,
        "nums": list(window),
        "sumP": sumP,
        "hwc": hwc_tuple,
        "dec": dec_tuple,
...
def _ticket_ok(ticket, *, target_hwc=None, target_dec=None, hot_set=None, warm_set=None, cold_set=None):
    import collections
    ...
    if target_hwc is not None:
        H = sum(1 for n in ticket if n in hot_set)
        W = sum(1 for n in ticket if n in warm_set)
        C = len(ticket) - H - W
        if (H, W, C) != tuple(target_hwc):
            return False

    if target_dec is not None:
        dc = collections.Counter(_decade_of_number(n) for n in ticket)
...
def generate_tickets_from_window(
    ...
    *,
    ...
    pool = sorted(set(window_nums))
    tickets = set()

    attempts = 0
    while len(tickets) < n_tickets and attempts < max_attempts:
        attempts += 1
        ticket = tuple(sorted(rnd.sample(pool, k=draw_size)))
        if not _ticket_ok(
            ticket,
...
def batch_learn_and_predict(
    *,
    ...
    seed=0,
    search_buckets=1,
    **_ignored_kwargs,   # defensive: won’t crash if wrapper passes extra junk
):
    """
    runs: list of payloads returned by core.main() for multiple dates.
    1) learn from runs WITH actual_main (found)
    2) pick a P-window for final_date using learned mapping
    3) generate tickets from that window subject to target_hwc/target_dec if provided
    """
    # split history vs final
    history = [r for r in runs if r.get("date") != final_date]
    final_run = next((r for r in runs if r.get("date") == final_date), None)
    if final_run is None:
        raise ValueError(f"final_date={final_date} not found in runs")

    # learn mappings from actuals
    learned = learn_pwindow_mappings_from_actuals(history)

    print("\n" + "="*80)
    print("LEARNED START-RANK VOTES")
    print("="*80)
    print(learned.get("start_rank_votes", []))

    # choose window for final run
    best = pick_prediction_window_from_history(final_run, learned, window_len=window_len, search_buckets=search_buckets)
...
def _build_rank_rows_from_scored_lists(res_pred, decade_of):
    """
    Builds rows sorted by P desc using res_pred['Hot_scored'/'Warm_scored'/'Cold_scored'].
    Output: list of dict rows:
      {rank, n, decade, cat, log_score, P}
    """
    rows = []

    def add(cat_letter, scored_list):
        for (n, log_score, P) in scored_list:
            rows.append({
                "n": int(n),
                "decade": int(decade_of(int(n))),
                "cat": cat_letter,  # 'H'/'W'/'C'
                "log_score": float(log_score),
                "P": float(P),
            })

    # DO NOT crash if missing — treat missing as empty
    add("H", res_pred.get("Hot_scored", []))
    add("W", res_pred.get("Warm_scored", []))
    add("C", res_pred.get("Cold_scored", []))
...
def _ensure_scored_buckets(res_pred, hot_set, warm_set, cold_set):
    """
    Ensure res_pred has:
      Hot, Warm, Cold: lists of numbers
      Hot_scored, Warm_scored, Cold_scored: list of (n, log_score, P)
    Using res_pred['P'] and res_pred['log_score'] dicts if present.
    """
    P_map = res_pred.get("P") or {}
    L_map = res_pred.get("log_score") or {}

    # Number lists
    res_pred["Hot"] = sorted(list(hot_set))
    res_pred["Warm"] = sorted(list(warm_set))
    res_pred["Cold"] = sorted(list(cold_set))

    def scored_list(nums):
        out = []
        for n in sorted(nums):
            out.append((n, float(L_map.get(n, 0.0)), float(P_map.get(n, 0.0))))
        # sort by P descending (this matches your rank logic)
        out.sort(key=lambda t: t[2], reverse=True)
        return out

    res_pred["Hot_scored"] = scored_list(hot_set)
    res_pred["Warm_scored"] = scored_list(warm_set)
    res_pred["Cold_scored"] = scored_list(cold_set)


def _build_rank_rows_from_pred(res_pred, decade_of, hot_set, warm_set, cold_set):
    """
    Build rank_rows as a list of dicts:
      {rank, n, decade, cat, log_score, P}
    Sorted by P desc.
    """
    P_map = res_pred.get("P") or {}
    ...
    rank_rows = []
    for i, (n, p) in enumerate(ranked, start=1):
        rank_rows.append({
            "rank": i,
            "n": int(n),
            "decade": int(decade_of(n)),
            "cat": cat_of(n),
            "log_score": float(L_map.get(n, 0.0)),
            "P": float(p),
        })
    return rank_rows, [int(n) for n, _ in ranked]
...
def main():
    ...
    # --- NEW: Suggest overrides (mapping only) ---
    # if not (PREDICTION_CONFIG and PREDICTION_CONFIG['APPLY_PREDICTION_OVERRIDES']):
    #     model_hwc_pred = (res_pred['h_target'], res_pred['w_target'], res_pred['c_target'])
    #     model_dec_pred = res_pred['dec_pred']
    #
    #     hwc_override, dec_factors_override, ev = suggest_overrides_from_mappings(
    #         model_hwc=model_hwc_pred,
    #         model_dec_pred=model_dec_pred,
    #         hwc_map=hwc_map,
    #         dec_map=dec_map
    #     )
    #
    #     print("\n=== OVERRIDE SUGGESTION (MAPPING ONLY / NO MEANS) ===")
    #     print(f"Model predicted H/W/C: {ev['model_hwc']}")
    #     print(f"Suggested HWC_OVERRIDE: {hwc_override}")
    #     print(f"HWC mapping counts for {ev['model_hwc']}: {ev['hwc_counts']}")
    #
    #     # print(f"\nModel decade-shape key (top2, low1): {ev['dec_key']}")
    #     # print(f"Chosen ACTUAL decade signature (dominant, missing): {ev['chosen_sig']}")
    #     # print(f"Decade mapping counts for {ev['dec_key']}: {ev['dec_counts']}")
    #
    #     print("\nPaste into wrapper:\n")
    #     print("core.PREDICTION_CONFIG.update({")
    #     print(f"  'HWC_OVERRIDE': {hwc_override},")
    #     # print("  'DECADE_FACTORS_OVERRIDE': {")
    #     # for d in sorted(dec_factors_override.keys()):
    #     #     print(f"    {d}: {dec_factors_override[d]:.2f},")
    #     # print("  },")
    #     print("})")
    top_predicted_tuples = res_pred.get("topN", [])
    # _ensure_scored_buckets(res_pred, log_score_map, P_map)
    rank_rows = _build_rank_rows_from_scored_lists(res_pred, decade_of)

    rank_rows = _build_rank_rows_from_scored_lists(res_pred, decade_of)
    # --- Normalize / enrich prediction payload for downstream learning ---
    # These sets already exist in your main() scope (you print them):
    Hot_set = res_pred["Hot_set"]
    Warm_set = res_pred["Warm_set"]
    Cold_set = res_pred["Cold_set"]

    # Ensure scored buckets exist (Hot_scored etc)
    _ensure_scored_buckets(res_pred, Hot_set, Warm_set, Cold_set)

    # Build rank_rows + ranked list from P/log_score
    rank_rows, ranked_nums = _build_rank_rows_from_pred(
        res_pred=res_pred,
        decade_of=decade_of,
        hot_set=Hot_set,
        warm_set=Warm_set,
    ...
    # Provide legacy aliases expected by the learner
    P_map = res_pred.get("P") or {}
    log_score_map = res_pred.get("log_score") or {}

    # Attach to res_pred so printing + later code can use consistent keys
    res_pred["P_map"] = P_map
    res_pred["log_score_map"] = log_score_map
    res_pred["rank_rows"] = rank_rows
    res_pred["ranked"] = ranked_nums

    return _make_run_payload(
        lottery=prediction_lottery_name,
        target_date=prediction_date,
        draw_size=prediction_draw_size,
        res_pred=res_pred,
        top_tuples=top_predicted_tuples,
        actual_main=actual_main,
        # normalized top-level keys (so downstream doesn't dig into res_pred)
        P_map=res_pred["P_map"],
        log_score_map=res_pred["log_score_map"],
        rank_rows=res_pred["rank_rows"],
        ranked=res_pred["ranked"],
    )
